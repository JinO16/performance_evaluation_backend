let SciPapers = require('../model/SciPapersMoudel').default;
let mongoose = require('mongoose');
module.exports = {
    //添加
    createSciPapers: function (reqParam) {
        return new Promise ((resolve,reject) => {
            ...
            let SciPapersData = new SciPapers(data);
            SciPapersData.save(function (err, data) {
                if (err) {
    ...
    getOwnSciPapers: (reqParam) => {
        return new Promise((resolve, reject) => {
            ...
            SciPapers.countDocuments(reqParam, function( err, count) {
                if (err) {
                    reject({ code: 500, message:'获取数据失败' + err.message})
                } else {
                    SciPapers.find(data, null, {skip:parseInt(skip), limit: parseInt(limit)}, function (err, docs) {
                        if (err) {
    ...
    deleteSciPapers: (reqParam) => {
        return new Promise((resolve,reject) => {
            ...
            SciPapers.findByIdAndRemove(data, function (err, data) {
                if (err) {
    ...
    updateSciPapers: (reqParam) => {
        return new Promise((resolve, reject) => {
            ...
            SciPapers.findByIdAndUpdate(data._id,updateData,{new: false}, function(err,data) {
                if (err) {
     ...
     getAllSciPapers: function (reqParam) {
        return new Promise((resolve, reject) => {
            let data = {}, skip, limit;
            if (reqParam.skip) {
                skip = reqParam.skip;
            ...
            SciPapers.countDocuments(data, function (err,count) {
                if (err) {
                    reject({ code: 500, message: '获取数据失败，' + err.message});
                } else {
                    SciPapers.find(data, null, {skip: parseInt(skip), limit: parseInt(limit)}, function(err, docs) {
