let ZygxStation = require('../model/ZygxStationMoudel');
...
module.exports = {
    ...
    createStation: function (reqParam) {
        return new Promise ((resolve,reject) => {
            ...
            let ZygxStationData = new ZygxStation(data);
            ZygxStationData.save(function (err,data) {
                if (err) {
    ...
    getStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            ZygxStation.countDocuments(data, function (err) {
                if (err) {
                    reject({ code: 500, message:'获取数据失败！' + err.message});
                } else {
                    ZygxStation.find(data, null, function (err,docs) {
                        if(err) {
    ...
    updateStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            ZygxStation.findByIdAndUpdate(data._id, updateData, function(err) {
                if (err) {
