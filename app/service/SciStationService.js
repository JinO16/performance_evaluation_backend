let SciStation = require('../model/SciStationMoudel');
...
module.exports = {
    ...
    createStation: function (reqParam) {
        return new Promise ((resolve,reject) => {
            ...
            console.log('科研岗位信息穿的数据data :>> ', data);
            let SciStationData = new SciStation(data);
            SciStationData.save(function (err,data) {
    ...
    getStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            SciStation.countDocuments(data, function (err) {
                if (err) {
                    reject({ code: 500, message:'获取数据失败！' + err.message});
                } else {
                    SciStation.find(data, null, function (err,docs) {
                        if(err) {
    ...
    updateStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            SciStation.findByIdAndUpdate(data._id, updateData, function(err) {
                if (err) {
