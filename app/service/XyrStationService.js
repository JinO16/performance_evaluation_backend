let XyrStation = require('../model/XyrStationMoudel');
...
module.exports = {
    ...
    createStation: function (reqParam) {
        return new Promise ((resolve,reject) => {
            ...
            let XyrStationData = new XyrStation(data);
            XyrStationData.save(function (err,data) {
                if (err) {
    ...
    getStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            XyrStation.countDocuments(data, function (err) {
                if (err) {
                    reject({ code: 500, message:'获取数据失败！' + err.message});
                } else {
                    XyrStation.find(data, null, function (err,docs) {
                        if(err) {
    ...
    updateStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            XyrStation.findByIdAndUpdate(data._id, updateData, function(err) {
                if (err) {
