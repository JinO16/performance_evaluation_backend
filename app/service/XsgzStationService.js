let XsgzStation = require('../model/XsgzStationMoudel');
...
module.exports = {
    ...
    createStation: function (reqParam) {
        return new Promise ((resolve,reject) => {
            ...
            let XsgzStationData = new XsgzStation(data);
            XsgzStationData.save(function (err,data) {
                if (err) {
    ...
    getStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            XsgzStation.countDocuments(data, function (err) {
                if (err) {
                    reject({ code: 500, message:'获取数据失败！' + err.message});
                } else {
                    XsgzStation.find(data, null, function (err,docs) {
                        if(err) {
    ...
    updateStation: function(reqParam) {
        return new Promise((resolve,reject) => {
            ...
            XsgzStation.findByIdAndUpdate(data._id, updateData, function(err) {
                if (err) {
