let SciAchievement = require('../model/SciAchievementMoudel').default;
let mongoose = require('mongoose');
module.exports = {
    //添加
    createSciAchievement: function (reqParam) {
        return new Promise ((resolve,reject) => {
            ...
            reqParam.sciAchievement && (data.sciAchievement = reqParam.sciAchievement);
            reqParam.submitTime && (data.submitTime = reqParam.submitTime);
            let SciAchievementData = new SciAchievement(data);
            SciAchievementData.save(function (err, data) {
                if (err) {
    ...
    getOwnSciAchievement: (reqParam) => {
        return new Promise((resolve, reject) => {
            ...
            SciAchievement.countDocuments(reqParam, function( err, count) {
                if (err) {
                    reject({ code: 500, message:'获取数据失败' + err.message})
                } else {
                    SciAchievement.find(data, null, {skip:parseInt(skip), limit: parseInt(limit)}, function (err, docs) {
                        if (err) {
    ...
    deleteSciAchievement: (reqParam) => {
        return new Promise((resolve,reject) => {
            ...
            SciAchievement.findByIdAndRemove(data, function (err, data) {
                if (err) {
    ...
    updateSciAchievement: (reqParam) => {
        return new Promise((resolve, reject) => {
            ...
            reqParam.sciAchievement && (updateData.sciAchievement = reqParam.sciAchievement);
            reqParam.submitTime && (updateData.submitTime = reqParam.submitTime);
            SciAchievement.findByIdAndUpdate(data._id,updateData,{new: false}, function(err,data) {
                if (err) {
     ...
     getAllSciAchievement: function (reqParam) {
        return new Promise((resolve, reject) => {
            let data = {}, skip, limit;
            if (reqParam.skip) {
                skip = reqParam.skip;
            ...
            SciAchievement.countDocuments(data, function (err,count) {
                if (err) {
                    reject({ code: 500, message: '获取数据失败，' + err.message});
                } else {
                    SciAchievement.find(data, null, {skip: parseInt(skip), limit: parseInt(limit)}, function(err, docs) {
